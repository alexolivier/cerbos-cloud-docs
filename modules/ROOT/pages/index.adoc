...
|**xref:synapse:ROOT:index.adoc[Cerbos Synapse]** — Data and integration layer
...
The https://github.com/cerbos/cerbos[open source Cerbos PDP] is a fully functional authorization engine that can run standalone, reading policies from a git repository, database, or local filesystem. No license or Hub account is required.

Cerbos Hub is an optional control plane that adds managed CI/CD, fleet-wide push-based updates, collaborative policy editing, embedded PDP support, and centralized audit log collection on top of the open source PDP. Switching to Hub requires only a configuration change — the PDP binary is the same.
...
* xref:synapse:ROOT:index.adoc[Synapse] — Authorization data enrichment and infrastructure integrations.
