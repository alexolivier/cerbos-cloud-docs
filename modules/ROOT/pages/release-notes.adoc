...
=== Limit on how many audit log exports can be started per hour

[#_audit_log_export_rate_limit]

Cerbos Hub now limits how many xref:audit-log-collection.adoc#_exporting_logs[audit log exports] can be started from a workspace within an hour. When the limit is reached, starting another export fails and nothing is added to the queue. An export stops counting towards the limit an hour after it was created, so you can start more once the earlier ones fall outside that window. Exports that are already queued or in progress are not affected.
...
=== Download the policies in a deployment

[#_download_deployment_policies]

The Policies tab of a xref:deployments.adoc#_downloading_policies[deployment] now has a *Download policies* button that gives you the policy files from the deployed build as a zip archive. The archive groups the files into a folder per contributing store, so you get an exact copy of what the deployment is running and can tell where each policy came from. This is useful for reviewing a build offline, comparing it against your repository, or keeping a record of what was deployed.
